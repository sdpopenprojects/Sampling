...
def nb_predict(trn_X, trn_y, tst_X, tst_y, effort, class_weight=None):
    modelNB = GaussianNB()
    modelNB.fit(trn_X, trn_y)
    tst_pred = modelNB.predict_proba(tst_X)
    Popt, Erecall, Eprecision, Efmeasure, PMI, IFA, recall, precision,  F1, Pf, AUC, MCC = evaluate_all(tst_pred[:, 1],effort, tst_y)
    return Popt, Erecall, Eprecision, Efmeasure, PMI, IFA, recall, precision,  F1, Pf, AUC, MCC
...
def rf_predict(trn_X, trn_y, tst_X, tst_y, effort, class_weight=None):
    ...
    tst_pred = modelRF.predict_proba(tst_X)
    Popt, Erecall, Eprecision, Efmeasure, PMI, IFA, recall, precision,  F1, Pf, AUC, MCC = evaluate_all(tst_pred[:, 1],effort, tst_y)
    return Popt, Erecall, Eprecision, Efmeasure, PMI, IFA, recall, precision,  F1, Pf, AUC, MCC

def save_results_to_csv(results,dataset,model):
    for key, value in results.items():
        ...
        outpath = f'./output/output-classifiers/{model}/{dataset}-{key}.csv'
...
def main(DATASETS,model):
    ...
    for j in range(10):
            ...
            for fold in range(totalFolds):
                ...
                #datapreprocessing
                trn_X, trn_y, tst_X, tst_y, effort = datapreprocessing(trn, tst)
                ...
                for method, sampler in sampling_methods.items():
                    ...
                    if model=='LR':
                        ...
                        result = nb_predict(n_X, n_y, tst_X, tst_y, effort, class_weight=None)
                    else:
                        result = rf_predict(n_X, n_y, tst_X, tst_y, effort, class_weight=None)
                    results[method] = np.vstack((results[method], result))
                    print(f"{method} is okay~")
            save_results_to_csv(results, dataset,model)
